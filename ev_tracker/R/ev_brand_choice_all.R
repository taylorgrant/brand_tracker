ev_brand_choice_all <- function() {
  ...
  for (brand in brands) {
    ...
    all_key_attrs <- dplyr::bind_rows(
      all_key_attrs,
      result$key_attrs |>  dplyr::mutate(brand = brand)
    )
