ev_tracker_figure <- function(dat, brand, filters, dataset_type){
  ...
  if ((brand == "BMW" & names(dat)[1] == "Unaided Awareness") | (brand != 'BMW' & names(dat)[1] == "Aided Awareness")) {
    tmp <- data.table::rbindlist(dat, idcol = "Category") |> 
      ...
      dplyr::mutate(Category = ifelse(Category == "Brand Momentum" & svy_q == "Ascending - Top 2 Box", paste0(Category, "\nTop 2 Box"), Category)) |>
  ...
  sample <- glue::glue("* Statistically significant lift at 95% confidence interval\n{channel_text}; {sub3} Sample ", 
                       "BMW EV Aware: Control = {round(tmp$`total_control`[3])}, Exposed = {round(tmp$`total_test`[3])}")
